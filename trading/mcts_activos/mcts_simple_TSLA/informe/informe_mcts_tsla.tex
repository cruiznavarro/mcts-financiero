...
Se implementa el método en Python y se evalúa sobre dos años de datos históricos de TSLA, comparando la estrategia MCTS frente a una estrategia pasiva \emph{Buy \& Hold} mediante: evolución de la cartera, alfa relativa, \emph{drawdown} absoluto y del alfa, retorno total y ratio de Sharpe. El error del estimador sigue la cota $\mathcal{O}(n^{-1/2})$ del Teorema del análisis del error (Aràndiga, §5), lo que justifica el valor $\texttt{ITERACIONES}=1000$ como compromiso entre precisión estadística y coste computacional.
...
\input{secciones/02_variables_aleatorias}
\input{secciones/03_monte_carlo}
\input{secciones/04_mdp_mcts}
