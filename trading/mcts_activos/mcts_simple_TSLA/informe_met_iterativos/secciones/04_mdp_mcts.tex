...
\begin{enumerate}
    \item[\textbf{(1) Selección.}] Partiendo de la raíz, se desciende por el árbol eligiendo en cada nodo interno la acción que maximiza~\eqref{eq:ucb1}, hasta alcanzar un nodo hoja. Esta fase se apoya exclusivamente en estadísticas ya recogidas en iteraciones previas y no consume recursos estocásticos.

    \item[\textbf{(2) Expansión.}] Si el nodo hoja no es terminal y ha sido visitado antes, se le añade un nuevo nodo hijo correspondiente a una acción aún no probada. Esta operación hace crecer el árbol en una unidad.

    \item[\textbf{(3) Rollout.}] Desde el nodo recién creado, se simula una trayectoria completa del MDP hasta el horizonte $H$, utilizando una política por defecto (en nuestro caso, uniforme sobre $\mathcal{A}$) y generando precios con el modelo GBM~\eqref{eq:gbm-exacta}. La trayectoria devuelve un retorno escalar $X$ que constituye \emph{una muestra} del estimador Monte Carlo estudiado en §\ref{sec:monte-carlo}.

    \item[\textbf{(4) Retropropagación.}] El retorno $X$ se propaga hacia la raíz actualizando, para cada nodo $(s,a)$ del camino recorrido en la fase de selección,
    \[
...
% ----------------------------------------------------------------------------
\subsection{Interpretación como método iterativo lineal}
\label{subsec:metodos-iterativos}

La demostración anterior establece la convergencia de MCTS por vía probabilística, apoyándose en la Ley Fuerte de los Grandes Números. Existe, sin embargo, una lectura complementaria que encuadra el método dentro de la teoría clásica de \emph{métodos iterativos} para la resolución de sistemas de ecuaciones (Aràndiga, Tema~6). Esta relectura no aporta nuevas conclusiones matemáticas, pero ilumina la estructura del algoritmo, justifica cuantitativamente la elección de la constante de exploración $c$ y proporciona un marco natural para discutir criterios de parada alternativos.

\subsubsection*{La actualización $Q/N$ como recurrencia lineal}

Fijemos una acción $a \in \mathcal{A}(s_0)$ en la raíz y denotemos por $q^{(k)}(a) := Q(s_0,a) / N(s_0,a)$ el estimador de su valor tras $k$ iteraciones totales del algoritmo. Cuando la iteración $k+1$ selecciona la acción $a$ y obtiene el retorno $X_{k+1}(a)$ en el \emph{rollout}, la actualización de las estadísticas $Q \leftarrow Q + X$, $N \leftarrow N + 1$ se traduce, tras dividir, en
\begin{equation}
    q^{(k+1)}(a) \;=\; \underbrace{\frac{N^{(k)}(a)}{N^{(k)}(a) + 1}}_{1 - \alpha_k} \, q^{(k)}(a) \;+\; \underbrace{\frac{1}{N^{(k)}(a) + 1}}_{\alpha_k} \, X_{k+1}(a),
    \label{eq:iterativo-q}
\end{equation}
con $\alpha_k := 1/(N^{(k)}(a)+1) \in (0,1]$. La expresión~\eqref{eq:iterativo-q} es formalmente idéntica al paso de un método iterativo lineal: $q^{(k+1)}(a)$ es combinación convexa del iterado anterior y de una \emph{entrada estocástica} $X_{k+1}(a)$ que hace el papel del término independiente $b$ del sistema $Ax = b$. La ``matriz de iteración'' se reduce aquí a un escalar,
\[
    Q_{\text{iter}}^{(k)} \;=\; 1 - \alpha_k \;=\; \frac{N^{(k)}(a)}{N^{(k)}(a) + 1},
\]
cuyo módulo satisface trivialmente $|Q_{\text{iter}}^{(k)}| < 1$ para todo $k$ tal que $N^{(k)}(a) \geq 0$, lo cual garantiza la condición $\rho < 1$ del Teorema de convergencia del Tema~6.

\subsubsection*{Ecuación del error}

Siguiendo el esquema de Aràndiga, definimos el error de estimación
\[
    e^{(k)}(a) \;:=\; q^{(k)}(a) - q^*(a), \qquad q^*(a) \;:=\; \mathbb{E}[R_T \mid s_0, a].
\]
Restando $q^*(a)$ en ambos miembros de~\eqref{eq:iterativo-q} y reordenando,
\begin{equation}
    e^{(k+1)}(a) \;=\; (1 - \alpha_k)\, e^{(k)}(a) \;+\; \alpha_k\, \bigl( X_{k+1}(a) - q^*(a) \bigr).
    \label{eq:error-iterativo}
\end{equation}
La expresión~\eqref{eq:error-iterativo} es la versión estocástica de la ecuación del error $e^{(k+1)} = Q e^{(k)}$ del Tema~6, con un término adicional de ruido $\alpha_k (X_{k+1} - q^*)$ de media nula. La LLN garantiza que, cuando $k \to \infty$, el ruido se promedia a cero y $e^{(k)} \to 0$ casi seguramente. Más aún, la varianza del error satisface
\[
    \mathrm{Var}\bigl( e^{(k)}(a) \bigr) \;=\; \frac{\sigma^2(a)}{N^{(k)}(a)},
\]
donde $\sigma^2(a) = \mathrm{Var}(X_i(a))$, lo que reproduce la cota $\mathcal{O}(N^{-1/2})$ ya establecida por el Teorema~\ref{teorema:error-mc} de §\ref{sec:monte-carlo}.

\subsubsection*{MCTS como Gauss--Seidel dirigido por UCB1}

En un método iterativo clásico, el patrón de actualización de las componentes del vector $x^{(k)} \in \mathbb{R}^n$ define el método:

\begin{itemize}
    \item \textbf{Jacobi} actualiza las $n$ componentes \emph{simultáneamente} en cada iteración, todas a partir del iterado anterior $x^{(k)}$.
    \item \textbf{Gauss--Seidel} actualiza \emph{una componente por iteración}, recorriéndolas en orden cíclico y aprovechando en cada paso los valores más recientes.
    \item \textbf{SOR} introduce un parámetro de relajación $\omega \in (0,2)$ que interpola entre sobrerelajación ($\omega > 1$) y subrelajación ($\omega < 1$) para minimizar el radio espectral $\rho(Q_\omega)$.
\end{itemize}

En el MCTS, el ``vector de iterados'' es $q^{(k)} = (q^{(k)}(a))_{a \in \mathcal{A}}$, una componente por acción. La observación clave es que cada iteración del algoritmo actualiza \emph{exactamente una componente}: aquella correspondiente a la acción $a^*$ seleccionada por UCB1 en la raíz. Las restantes $|\mathcal{A}| - 1$ componentes permanecen inalteradas. Esto identifica al MCTS como una variante de \textbf{Gauss--Seidel} en la cual el orden cíclico de recorrido se sustituye por una \emph{política adaptativa de selección}, dictada por la fórmula~\eqref{eq:ucb1}. La Figura~\ref{fig:patron-actualizacion} ilustra gráficamente esta distinción para una simulación de $30$ iteraciones sobre el espacio de $11$ acciones del problema TSLA.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figuras/patron_actualizacion.png}
    \caption{Patrones de actualización de las componentes $q^{(k)}(a)$ en tres métodos iterativos. En Jacobi~(a), cada iteración actualiza todas las acciones de forma paralela. En Gauss--Seidel~(b), se actualiza una única acción por iteración, en orden cíclico. En MCTS con UCB1~(c), se actualiza una acción por iteración elegida adaptativamente: tras una fase inicial de exploración uniforme (primeras $|\mathcal{A}|$ iteraciones, en las que todas las acciones tienen $N=0$ y reciben prioridad máxima), la política concentra las visitas en las acciones más prometedoras (aquí, \texttt{COMPRAR\_100} y \texttt{VENDER\_100}).}
    \label{fig:patron-actualizacion}
\end{figure}

\subsubsection*{El papel de $c$ como parámetro de relajación}

La analogía con SOR permite interpretar con precisión la constante de exploración $c$ de la fórmula UCB1~\eqref{eq:ucb1}. El parámetro $\omega$ del SOR controla el peso relativo entre el iterado anterior y la corrección de Gauss--Seidel: un $\omega$ óptimo $\bar\omega$ minimiza el radio espectral $\rho(Q_\omega)$ y por tanto la velocidad de convergencia. De manera paralela, $c$ regula el equilibrio entre:
\begin{itemize}
    \item \textbf{Explotación} ($c$ pequeño, análogo a $\omega < 1$): el algoritmo visita reiteradamente la acción con mejor $q^{(k)}$, actualizando sólo una componente y ralentizando la actualización del resto. Riesgo: quedar atrapado en óptimos locales si el valor empírico temprano es engañoso.
    \item \textbf{Exploración} ($c$ grande, análogo a $\omega > 1$): el algoritmo reparte las iteraciones entre todas las acciones, garantizando $N(a) \to \infty$ para todas, pero a costa de visitar menos las acciones realmente prometedoras.
\end{itemize}

El valor canónico $c = \sqrt{2}$, adoptado en \texttt{mcts\_simple\_TSLA.py} (línea~382), corresponde en la analogía SOR al caso $\omega = 1$ (Gauss--Seidel puro) y está justificado por la cota de regret $\mathcal{O}(\ln n)$ de Auer, Cesa-Bianchi y Fischer~\cite{auer2002}. Valores $c \neq \sqrt{2}$ introducirían una sobrerelajación o subrelajación sobre el balance explotación--exploración; en este trabajo mantenemos $c = \sqrt{2}$.

\subsubsection*{Convergencia empírica y ventaja de UCB1}

La Figura~\ref{fig:convergencia-iterativa} valida empíricamente la cota $|e^{(k)}(a)| = \mathcal{O}(N^{-1/2})$ derivada de~\eqref{eq:error-iterativo}, en una simulación de $n=5\,000$ iteraciones sobre un problema sintético con $11$ acciones y valores reales conocidos. El panel~(a) muestra el decaimiento log-log del error para cada acción, comparado con la cota teórica $\sigma/\sqrt{N}$; el panel~(b) revela la estructura de visitas final: UCB1 concentra más del $90\,\%$ de las iteraciones en las dos acciones óptimas (aquí \texttt{C100} y \texttt{V100}, con valores reales $q^*=1{.}0$ y $q^*=0{.}7$), reservando sólo un puñado de visitas para el resto. Este reparto no uniforme es precisamente lo que distingue al MCTS de un Gauss--Seidel cíclico: la información acumulada en iteraciones previas \emph{retroalimenta} la elección del siguiente paso iterativo, acelerando la localización del óptimo global sin perder la garantía asintótica de convergencia.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figuras/convergencia_iterativa.png}
    \caption{Convergencia del error del estimador~\eqref{eq:error-iterativo} en una simulación MCTS con $n=5\,000$ iteraciones. \textbf{(a)} Error empírico $|e^{(k)}(a)| = |q^{(k)}(a) - q^*(a)|$ frente al número de visitas $N(a)$, en escala doblemente logarítmica, comparado con la cota teórica $\sigma/\sqrt{N}$ (línea discontinua negra). La pendiente $-1/2$ se respeta para todas las acciones. \textbf{(b)} Número final de visitas por acción: UCB1 concentra las iteraciones en las dos acciones óptimas (\texttt{C100} y \texttt{V100}), con valores reales $q^*=1{.}0$ y $q^*=0{.}7$, dejando a las demás cerca del mínimo necesario para descartarlas como subóptimas.}
    \label{fig:convergencia-iterativa}
\end{figure}

\subsubsection*{Criterio de parada adaptativo}

En el Tema~6, un criterio de parada natural para un método iterativo con $\|Q\| < 1$ es detener las iteraciones cuando la diferencia entre iterados consecutivos es pequeña:
\begin{equation}
    \|x^{(k)} - x^{(k-1)}\| \;\leq\; \varepsilon \cdot \frac{1 - \|Q\|}{\|Q\|}.
    \label{eq:parada-adaptativa}
\end{equation}
En el caso del MCTS, el programa \texttt{mcts\_simple\_TSLA.py} utiliza un criterio \emph{fijo} de $\texttt{ITERACIONES} = 5\,000$ por decisión. Una alternativa adaptativa, inspirada en~\eqref{eq:parada-adaptativa}, sería detener el algoritmo cuando la acción mejor estimada se estabilice:
\[
    \left| q^{(k)}(a^*) - q^{(k-1)}(a^*) \right| \;\leq\; \varepsilon,
    \qquad a^* = \arg\max_a q^{(k)}(a).
\]
Equivalentemente, puede usarse el error estándar $\sigma(a)/\sqrt{N(a)} \leq \varepsilon$, que es la traducción probabilística directa de~\eqref{eq:parada-adaptativa}. No implementamos esta variante en el presente trabajo, pero queda señalada como línea de mejora natural en §\ref{sec:discusion}.

\subsubsection*{Resumen de la analogía}

La Tabla~\ref{tab:analogia-iterativo} sintetiza las correspondencias establecidas entre los elementos del Tema~6 (métodos iterativos lineales) y los del MCTS estudiado en este informe.

\begin{table}[H]
    \centering
    \caption{Correspondencia entre métodos iterativos clásicos (Aràndiga, Tema~6) y MCTS.}
    \label{tab:analogia-iterativo}
    \begin{tabular}{@{}ll@{}}
        \toprule
        \textbf{Método iterativo clásico} & \textbf{Elemento correspondiente en MCTS} \\
        \midrule
        Iterado $x^{(k)} \in \mathbb{R}^n$ & Vector $q^{(k)} = (q^{(k)}(a))_{a\in\mathcal{A}}$ \\
        Solución $x^*$ con $Ax^* = b$ & Valor óptimo $q^*(a) = \mathbb{E}[R_T \mid s_0, a]$ \\
        Término independiente $b$ & Retorno estocástico $X_{k+1}(a)$ del \emph{rollout} \\
        Matriz de iteración $Q = M^{-1}N$ & Escalar $1 - \alpha_k = N(a)/(N(a)+1)$ \\
        Radio espectral $\rho(Q) < 1$ & $|1 - \alpha_k| < 1 \;\forall k$ (trivial) \\
        Error $e^{(k)} = x^{(k)} - x^*$ & Sesgo del estimador $q^{(k)}(a) - q^*(a)$ \\
        Actualización cíclica (Gauss--Seidel) & Selección adaptativa por UCB1 \\
        Parámetro de relajación $\omega$ (SOR) & Constante de exploración $c$ de UCB1 \\
        Criterio de parada $\|x^{(k)}-x^{(k-1)}\| < \varepsilon$ & Error estándar $\sigma(a)/\sqrt{N(a)} < \varepsilon$ \\
        \bottomrule
    \end{tabular}
\end{table}

\noindent El MCTS es, en esencia, un método iterativo estocástico componente a componente, con un mecanismo de selección adaptativa de componente basado en UCB1. Esta lectura integra el algoritmo en el marco del Tema~6 de la asignatura y muestra que los principios cualitativos allí establecidos ---convergencia controlada por una ``matriz'' de iteración con $\rho < 1$, velocidad de convergencia vinculada al espectro, equilibrio entre sobrerelajación y estabilidad--- admiten una traducción probabilística directa al contexto del aprendizaje por refuerzo mediante simulación.
